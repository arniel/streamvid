...
class SickBeard_DB extends SQLite3{
    ...
    function get_all_episodes(){
        //$rows = $this->query("SELECT * FROM tv_episodes WHERE location!='' AND showid = ".$show_id);
        $rows = $this->query("SELECT * FROM tv_episodes WHERE location!='' AND location LIKE '%mp4%' ORDER BY name");
        $result = array();
        while($row = $rows->fetchArray(SQLITE3_ASSOC) ){
            /*?><a href="<?=$_SERVER['PHP_SELF']?>?v=<?=$row['episode_id']?>"><?=$row['season']?>x<?=$row['episode']?> - <?=$row['name']?></a><br><?*/
            array_push($result,$row);
	...
	function get_random_eid(){
		...
		shuffle($result);
		return isset($result[0])?$result[0]['episode_id']:null;
	}
	...
	function upcoming_episodes($limit = NULL){
		...
		$rows = $this->query("
			...
			FROM tv_episodes e, tv_shows s
			WHERE s.tvdb_id = e.showid
				AND e.airdate >= ".datetime_to_ordinal(new DateTime("today"))."
				AND e.airdate < ".datetime_to_ordinal(new DateTime("today + 30days"))."
